...
import type { ApprovalInstance, ApprovalTaskStatus } from '@/components/approval/api/approval'
import { showIncompletedUserName } from './utils'
import StatusTag from './StatusTag'
import type { ApprovalInstanceStatus } from '@/api/types/approval'
  ...
  setup(props) {
    const statusText = computed(() => {
      return ['rejected', 'canceled', 'approved'].includes(props.data.status)
        ? h(
            StatusTag(
              props.data.status as ApprovalInstanceStatus[keyof ApprovalInstanceStatus],
              'Instance',
            ),
          )
        : h(
            StatusTag(
              props.data.taskStatus as ApprovalTaskStatus[keyof ApprovalTaskStatus],
              'Task',
            ),
          )
    })
