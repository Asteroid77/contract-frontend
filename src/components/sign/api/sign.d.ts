...
import type { OssCallbackDTO } from '../../file/api/file-storage'
import type { PreviewTypeEnum } from '../constant/enum'
 ...
 * 2: 签约
 */
export type ServiceAgreementStatus = 1 | 2
...
export interface ServiceAgreementBaseVO
  extends Omit<ServiceAgreement, 'creator' | 'updatedTime' | 'createdTime'> {
  /**
   * 营销户号集合
   */
...
export interface ServiceAgreementAttaches {
  contractScanFiles: OssCallbackDTO[]
  billFiles: OssCallbackDTO[]
  supplementaryAttachmentFiles: OssCallbackDTO[]
}
...
export type ServiceAgreementAttachesVO = {
  /** 近一个月电费单 */
  billFiles: OssCallbackDTO[]
  /** 额外附件 */
  supplementaryAttachmentFiles: OssCallbackDTO[]
 ...
 * 预览附件的返回 VO
 */
export interface PreviewAttachmentsVO extends Record<string, OssCallbackDTO[]> {
  newFiles: ServiceAgreementAttachesVO
  oldFiles: ServiceAgreementAttachesVO
}
