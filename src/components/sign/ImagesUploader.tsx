...
import type { FileCategoryCode, OssCallbackDTO } from '@/components/file/api/file-storage'
import { message } from '@/_utils/discrete_naive_api'
import { useUploadFileMutation } from '@/components/sign/hooks/useSignService'
...
export default defineComponent({
  ...
  props: {
    ...
    fileCategory: {
      type: String as PropType<FileCategoryCode>,
      required: true,
    },
    maxFiles: {
      type: Number,
  ...
  setup(props, { emit }) {
    ...
    const customRequest = ({ file, onFinish, onError, onProgress }: UploadCustomRequestOptions) => {
      ...
      uploadFileMutation.mutate(
        ...
        {
          onSuccess: (response) => {
            const uploadedFileData: OssCallbackDTO = response.data
