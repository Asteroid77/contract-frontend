...
export const useDashboardFormatters = (locale: Ref<string>, t: Translate) => {
  const numberLocale = computed(() => (locale.value.toLowerCase().startsWith('zh') ? 'zh-CN' : 'en-US'))

  const formatInteger = (value: number): string => new Intl.NumberFormat(numberLocale.value).format(value)
