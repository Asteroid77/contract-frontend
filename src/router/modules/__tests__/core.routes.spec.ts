...
describe('core route modules config', () => {
  ...
  it('manage routes preserve transition entry and nested child', () => {
    ...
    expect(manageUser?.meta?.ability).toEqual({ action: 'read', subject: 'user-page' })
    expect(manageUserDetail?.meta?.ability).toEqual({ action: 'read', subject: 'user' })
    expect(manageUserEdit?.meta?.ability).toEqual({ action: 'update', subject: 'user' })
