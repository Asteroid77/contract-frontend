...
import type { RoleAssignRequest } from '@/api/types/user-role'
...
export const useAssignedUsersByRole = (
  ...
  return useQuery({
    ...
    queryFn: () => {
      ...
      return userRoleApi.getAssignedUsersByRoleId(id)
    },
    enabled: computed(() => {
      const id = unref(roleId)
...
export const useAssignRoleToUsers = () => {
  ...
  return useMutation({
    mutationFn: (data: RoleAssignRequest) => userRoleApi.assign(data),
    onSuccess: () => {
      // 也可以使角色列表缓存失效
