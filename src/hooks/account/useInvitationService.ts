import { invitationApi } from '@/components/invitation/api/invitation.api'
import type {
  InvitationCode,
  InvitationUpdateRequest,
} from '@/components/invitation/api/invitation'
...
export const useInvitationCodeListQuery = () => {
  return useQuery<
    ServerResponse<InvitationCode[]>,
    AxiosError<ServerResponse<unknown>>,
    InvitationCode[]
  >({
    queryKey: invitationKeys.lists(),
    queryFn: () => invitationApi.getInvitationCodeList(),
  })
}
...
export const useInvitatedCountQuery = () => {
  return useQuery<ServerResponse<number>, AxiosError<ServerResponse<unknown>>, number>({
    queryKey: invitationKeys.count(),
    queryFn: () => invitationApi.getInvitatedCount(),
  })
}
...
export const useCreateInvitationCodeMutation = () => {
  ...
  return useMutation({
    mutationFn: () => invitationApi.createInvitationCode(),
    onSuccess: () => {
...
export const useUpdateInvitationCodeMutation = (successCallback: () => void) => {
  ...
  return useMutation({
    ...
    mutationFn: (params: InvitationUpdateRequest[]) => invitationApi.updateInvitationCode(params),
    onSuccess: () => {
      // 更新成功后，同样让列表缓存失效
      queryClient.invalidateQueries({ queryKey: invitationKeys.lists() })
      successCallback()
...
export const useDeleteInvitationCodeMutation = (successCallback: () => void) => {
  ...
  return useMutation({
    mutationFn: (params: number[]) => invitationApi.deleteInvitationCode(params),
    onSuccess: () => {
      // 更新成功后，同样让列表缓存失效
      queryClient.invalidateQueries({ queryKey: invitationKeys.lists() })
      successCallback()
